...
void MainWindow::on_actionOpen_Left_Hand_Mod_romfs_triggered()
{
    ...
    if (dirPath.filename() != "romfs")
    {
        em->showMessage("INVALID PATH!");
        return;
    }
    if (!std::filesystem::is_regular_file(dirPath / "pokemon/catalog/catalog/poke_resource_table.trpmcatalog"))
    {
        em->showMessage("INVALID PATH!");
        return;
    }
...
void MainWindow::on_actionSet_Output_Will_create_romfs_subfolder_triggered()
{
    ...
    if (!ui->label_5->text().toStdString().empty())
...
void MainWindow::on_pushButton_clicked()
{
    ...
    cb.add_version(vInf);
    cb.add_table(entries);
    auto offset = cb.Finish();
    fbb.Finish(offset);
    ...
    std::ofstream out(std::filesystem::path(ui->label_5->text().toStdString()) / "romfs/pokemon/catalog/catalog/poke_resource_table.trpmcatalog", std::ios::binary);
...
{
    setEnabled(false);
    aw = std::make_shared<AboutWindow>(this);
    aw->show();
}
